...
def _tail_pin(expected_tail_seq: object, expected_tail_hash: object) -> tuple[int, str] | None:
    """Validate an optional exact tail commitment without exposing it in output."""
    if expected_tail_seq is None and expected_tail_hash is None:
        return None
    try:
        seq = int(expected_tail_seq)
    except (TypeError, ValueError) as exc:
        raise CaptureError("CAPTURE_TAIL_PIN_INVALID") from exc
    tail_hash = str(expected_tail_hash or "").strip().lower()
    if isinstance(expected_tail_seq, bool) or seq < 1 or not _SHA256_RE.fullmatch(tail_hash):
        raise CaptureError("CAPTURE_TAIL_PIN_INVALID")
...
def _validated_export(
    body: bytes,
    *,
    expected_tail_seq: object = None,
    expected_tail_hash: object = None,
) -> tuple[bytes, str]:
    try:
    ...
    if hashlib.sha256(payload).hexdigest() != sha:
        raise CaptureError("CAPTURE_SHA256_MISMATCH")
    pin = _tail_pin(expected_tail_seq, expected_tail_hash)
    if pin is not None:
        raw_seq = value.get("tail_seq")
        response_hash = str(value.get("tail_event_hash") or "").strip().lower()
        if isinstance(raw_seq, bool) or not isinstance(raw_seq, int) or not _SHA256_RE.fullmatch(response_hash):
            raise CaptureError("CAPTURE_RESPONSE_INVALID")
        if (raw_seq, response_hash) != pin:
            raise CaptureError("CAPTURE_TAIL_PIN_MISMATCH")
    return payload, sha
...
def capture_and_encrypt(
    *,
    ...
    diagnostic_intent = str(intent or "").strip()
    if len(diagnostic_intent) < 32:
        raise CaptureError("CAPTURE_INTENT_INVALID")
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "X-CBI-Diagnostic-Intent": diagnostic_intent,
        },
        method="POST",
    )
    try:
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--expected-tail-seq", type=int)
    parser.add_argument("--expected-tail-hash")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    intent = str(os.environ.get("CBI_V64_C279_CAPTURE_INTENT") or "").strip()
    try:
        envelope = capture_and_encrypt(
            ...
            expected_tail_seq=args.expected_tail_seq,
            expected_tail_hash=args.expected_tail_hash,
        )
