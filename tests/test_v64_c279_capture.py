...
class V64C279CaptureIntentTests(unittest.TestCase):
    ...
    def test_tail_pin_is_validated_before_any_crypto_dependency_or_ciphertext(self):
        capture = self._module()
        response = {
            "schema": "cbi.v64-c279-single-session-export.v1",
            ...
            "tail_seq": 2,
            "tail_event_hash": "2" * 64,
            "payload_encoding": "base64",
            "payload": base64.b64encode(PLAINTEXT).decode("ascii"),
        }
        with self.assertRaises(capture.CaptureError) as caught:
            capture._validated_export(
                json.dumps(response).encode("utf-8"),
                expected_tail_seq=1,
                expected_tail_hash="1" * 64,
            )
        self.assertEqual(str(caught.exception), "CAPTURE_TAIL_PIN_MISMATCH")
...
class V64C279CaptureTests(unittest.TestCase):
    ...
    def test_exact_pinned_capture_rejects_wrong_tail_before_encryption_or_artifact_metadata(self):
        """A candidate capture binds the live response to its declared tail pin.

        The envelope deliberately carries no investigation id, tail hash, route,
        or plaintext.  Those pins stay in the verifier bridge, while a mismatch
        is rejected before ciphertext is written.
        """
        capture = self._module()
        _private, public_b64 = self._recipient()
        with _export_server(PLAINTEXT, tail_seq=2, tail_event_hash="2" * 64) as (base_url, _observed):
            with self.assertRaises(capture.CaptureError) as caught:
                capture.capture_and_encrypt(
                    base_url=base_url,
                    bearer=BEARER,
        ...
        with _export_server(PLAINTEXT, tail_seq=1, tail_event_hash="1" * 64) as (base_url, _observed):
        ...
        self.assertEqual(set(envelope), {
            "schema", "ephemeral_public_key", "salt", "nonce", "snapshot_sha256", "ciphertext",
        })
        serialized = json.dumps(envelope, sort_keys=True)
        self.assertNotIn("tail_event_hash", serialized)
        self.assertNotIn("tail_seq", serialized)
