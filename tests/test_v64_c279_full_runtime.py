...
def _positive_route_precondition(routes: object, account_id: object) -> dict[str, int]:
    ...
    `kind`/`channel` and `observation_id`/`information_id` are the only
    aliases used by the candidate's documented canonical Route View. In
    particular, legacy `owner_id` is not an ownership alias here: a positive
    acceptance case must carry the explicit outbound-route ownership fields.
    """
    ...
    for route in routes if isinstance(routes, list) else []:
        ...
        canonical_identifier = (
            ...
            or str(route.get("information_id") or "").strip()
        )
        if not canonical_identifier:
            continue
        if route.get("masked") is True or route.get("guessed") is True:
            continue
        if channel in {"WHATSAPP", "ZALO"} and route.get("channel_proof") is not True:
            continue
        fully_qualified += 1
        if isinstance(route.get("named_person"), str) and route["named_person"].strip():
            named_qualified += 1
    return {
        ...
        with tempfile.TemporaryDirectory(prefix="cbi-v64-positive-route-") as temp_dir:
            ...
            with patch.dict(os.environ, runtime_env, clear=False):
                ...
                eligible_named_routes = [
                    ...
                    if _positive_route_precondition([row], account_id)["named_qualified_route_count"] == 1
                    and (
                        (
                            str(row.get("route_source") or row.get("source") or "").strip().upper() == "COMPILED_OBSERVATION"
                            and str(row.get("observation_id") or "").strip() in valid_named_observations
                        )
                        or (
                            str(row.get("route_source") or row.get("source") or "").strip().upper()
                            in {"INFORMATION_RECORD", "INFORMATION_HISTORY", "INFORMATION_HISTORY_DERIVED"}
                            and str(row.get("information_id") or "").strip() in valid_information_routes
                        )
                    )
                ]
