...
class ResearchOrchestrationTests(unittest.TestCase):
    ...
    def test_lower_named_route_with_conflicting_owner_aliases_is_not_projected(self):
        base = {
            "route_source": "COMPILED_OBSERVATION", "observation_id": "OBS-CONFLICT",
            "owner_id": "SYNTH-ACCOUNT-ROUTE-001", "channel": "EMAIL",
            "value": "named@example.invalid", "named_person": "Private Name",
            "verified": True, "current": True, "route_scope": "BUYER_DIRECT",
            "evidence_ids": ["EVD-CONFLICT"],
        }
        variants = {
            "wrong_explicit_entity": {
                **base, "owned_by_account": True,
                "owner_entity_id": "OTHER",
            },
            "false_explicit_ownership": {
                **base, "owned_by_account": False,
                "owner_entity_id": "SYNTH-ACCOUNT-ROUTE-001",
            },
            "one_sided_entity": {
                **base, "owner_entity_id": "SYNTH-ACCOUNT-ROUTE-001",
            },
            "one_sided_owned_flag": {
                **base, "owned_by_account": True,
            },
            "conflicting_legacy_alias": {
                **base, "owned_by_account": True,
                "owner_entity_id": "SYNTH-ACCOUNT-ROUTE-001", "owner_id": "OTHER",
            },
        }
        for name, row in variants.items():
            with self.subTest(name=name):
                self.runtime.outreach_result = {
                    ...
                    "canonical_route_sources": ["COMPILED_OBSERVATION"],
                    "block_reasons": ["DECISION_SATURATION_CLOSURE_REQUIRED"],
                    "sends_message": False, "canonical_route_view": [row],
                }
                result = self.runtime.evaluate_outreach_readiness(self.args)
                self.assertEqual(result["canonical_route_view"], [])

    def test_conflicting_source_aliases_are_not_canonicalized_or_ready(self):
        base = {
            "kind": "EMAIL", "value": "named@example.invalid", "verified": True,
            "current": True, "owned_by_account": True,
            "owner_entity_id": "SYNTH-ACCOUNT-ROUTE-001", "masked": False,
            "guessed": False, "evidence_ids": ["EVD-SOURCE-CONFLICT"],
            ...
            (
                "compiled_then_information",
                {
                    **base,
                    ...
                    "observation_id": "OBS-SOURCE-CONFLICT",
                },
                {"valid_named_route_observation_ids": ["OBS-SOURCE-CONFLICT"]},
            ),
            (
                "information_then_compiled",
                {
                    **base,
                    ...
                    "information_id": "INFO-SOURCE-CONFLICT",
                },
                {"valid_information_route_ids": ["INFO-SOURCE-CONFLICT"]},
            ),
        )
        for name, row, lower_ids in cases:
            with self.subTest(name=name):
                self.assertFalse(self.runtime._route_lane_is_unambiguous(row))
                self.runtime.outreach_result = {
                    "outreach_readiness": "NAMED_ROUTE_READY",
                    "readiness": "NAMED_ROUTE_READY",
                    "valid_company_route_observation_ids": [],
                    "valid_named_route_observation_ids": lower_ids.get(
                        "valid_named_route_observation_ids", []
                    ),
                    "valid_information_route_ids": lower_ids.get(
                        "valid_information_route_ids", []
                    ),
                    "canonical_route_view": [row],
                    "canonical_route_sources": ["COMPILED_OBSERVATION", "INFORMATION_HISTORY"],
                    "block_reasons": [],
                    "sends_message": False,
                }
                result = self.runtime.evaluate_outreach_readiness(self.args)
                ...
                self.assertIn(
                    "VERIFIED_ACCOUNT_OWNED_ROUTE_REQUIRED",
                    result["block_reasons"],
                )
                self.assertFalse(result["sends_message"])

    def test_final_named_ids_are_rebuilt_and_company_only_view_cannot_keep_named_readiness(self):
        account_id = "SYNTH-ACCOUNT-ROUTE-001"
        company = {
            "route_source": "COMPILED_OBSERVATION", "observation_id": "OBS-COMPANY",
            "kind": "EMAIL", "value": "company@example.invalid", "verified": True,
        ...
        named = {
            "route_source": "COMPILED_OBSERVATION", "observation_id": "OBS-NAMED",
            "kind": "EMAIL", "value": "named@example.invalid", "named_person": "Private Name",
            "verified": True, "current": True, "owned_by_account": True,
            "owner_entity_id": account_id, "route_scope": "BUYER_DIRECT",
            "evidence_ids": ["EVD-NAMED"],
        }
        ...
        self.runtime.outreach_result = {
            "outreach_readiness": "NAMED_ROUTE_READY", "readiness": "NAMED_ROUTE_READY",
            "valid_company_route_observation_ids": ["OBS-COMPANY", "OBS-NAMED"],
            "valid_named_route_observation_ids": ["OBS-NAMED", "OBS-STALE"],
            "valid_information_route_ids": [],
            "canonical_route_sources": ["COMPILED_OBSERVATION"],
            "block_reasons": [], "sends_message": False,
            "canonical_route_view": [company, named],
        ...
        self.runtime.outreach_result["canonical_route_view"] = [company]
        self.runtime.outreach_result["valid_named_route_observation_ids"] = ["OBS-STALE"]
        result = self.runtime.evaluate_outreach_readiness(self.args)
        self.assertEqual(result["canonical_route_view"], [company])
        self.assertEqual(result["valid_named_route_observation_ids"], [])
        self.assertEqual(result["outreach_readiness"], "COMPANY_ROUTE_READY")
        self.assertEqual(result["readiness"], "COMPANY_ROUTE_READY")

    def test_lower_blocked_opted_out_state_cannot_be_upgraded_by_final_company_route(self):
        company = {
            "route_source": "COMPILED_OBSERVATION", "observation_id": "OBS-OPTOUT",
            "kind": "EMAIL", "value": "company@example.invalid", "verified": True,
            "current": True, "owned_by_account": True,
            "owner_entity_id": "SYNTH-ACCOUNT-ROUTE-001", "route_scope": "BUYER_DIRECT",
            "evidence_ids": ["EVD-OPTOUT"],
        }
        self.runtime.outreach_result = {
            "outreach_readiness": "BLOCKED", "readiness": "BLOCKED",
            "valid_company_route_observation_ids": [],
            "valid_named_route_observation_ids": [], "valid_information_route_ids": [],
            "canonical_route_sources": ["COMPILED_OBSERVATION"],
            "block_reasons": ["ACCOUNT_OPTED_OUT"], "sends_message": False,
            "canonical_route_view": [company],
        }

        result = self.runtime.evaluate_outreach_readiness(self.args)
        self.assertEqual(result["outreach_readiness"], "BLOCKED")
        self.assertEqual(result["readiness"], "BLOCKED")
        self.assertIn("ACCOUNT_OPTED_OUT", result["block_reasons"])
        self.assertNotEqual(result["outreach_readiness"], "COMPANY_ROUTE_READY")
        self.assertFalse(result["sends_message"])

    def test_lower_blocked_identity_route_state_cannot_be_upgraded_without_final_route(self):
        self.runtime.outreach_result = {
            "outreach_readiness": "BLOCKED", "readiness": "BLOCKED",
            "valid_company_route_observation_ids": [],
            "valid_named_route_observation_ids": [], "valid_information_route_ids": [],
            "canonical_route_sources": [],
            "block_reasons": ["IDENTITY_AND_ROUTE_NOT_READY"], "sends_message": False,
            "canonical_route_view": [],
        }
        ...
        self.assertEqual(result["outreach_readiness"], "BLOCKED")
        self.assertEqual(result["readiness"], "BLOCKED")
        self.assertIn("IDENTITY_AND_ROUTE_NOT_READY", result["block_reasons"])
        self.assertIn("VERIFIED_ACCOUNT_OWNED_ROUTE_REQUIRED", result["block_reasons"])
        self.assertFalse(result["sends_message"])

    def test_lower_identity_only_with_stale_route_blocker_can_upgrade_to_company_readiness(self):
        company = {
            "route_source": "COMPILED_OBSERVATION", "observation_id": "OBS-COMPANY-ONLY",
            "kind": "EMAIL", "value": "company@example.invalid", "verified": True,
            "current": True, "owned_by_account": True,
            "owner_entity_id": "SYNTH-ACCOUNT-ROUTE-001", "route_scope": "BUYER_DIRECT",
            "evidence_ids": ["EVD-COMPANY-ONLY"],
        }
        self.runtime.outreach_result = {
            "outreach_readiness": "IDENTITY_ONLY", "readiness": "IDENTITY_ONLY",
            "valid_company_route_observation_ids": [],
            "valid_named_route_observation_ids": [], "valid_information_route_ids": [],
            "canonical_route_sources": [],
            "block_reasons": ["VERIFIED_ACCOUNT_OWNED_ROUTE_REQUIRED"],
            "sends_message": False, "canonical_route_view": [company],
        }

        result = self.runtime.evaluate_outreach_readiness(self.args)
        self.assertEqual(result["outreach_readiness"], "COMPANY_ROUTE_READY")
        self.assertEqual(result["readiness"], "COMPANY_ROUTE_READY")
        self.assertIn("NAMED_ROUTE_NOT_PROVEN", result["block_reasons"])
        self.assertNotIn("VERIFIED_ACCOUNT_OWNED_ROUTE_REQUIRED", result["block_reasons"])
        self.assertFalse(result["sends_message"])

    def test_lower_send_level_with_final_company_only_route_is_capped_and_keeps_real_blockers(self):
        company = {
            "route_source": "COMPILED_OBSERVATION", "observation_id": "OBS-COMPANY-CAP",
            "kind": "EMAIL", "value": "company@example.invalid", "verified": True,
            "current": True, "owned_by_account": True,
            "owner_entity_id": "SYNTH-ACCOUNT-ROUTE-001", "route_scope": "BUYER_DIRECT",
            "evidence_ids": ["EVD-COMPANY-CAP"],
        }
        for lower_readiness in ("NAMED_ROUTE_READY", "FOLLOW_UP_READY", "SEND_READY"):
            with self.subTest(lower_readiness=lower_readiness):
                self.runtime.outreach_result = {
                    "outreach_readiness": lower_readiness, "readiness": lower_readiness,
                    "valid_company_route_observation_ids": [],
                    "valid_named_route_observation_ids": [], "valid_information_route_ids": [],
                    "canonical_route_sources": ["COMPILED_OBSERVATION"],
                    "block_reasons": [
                        "VERIFIED_ACCOUNT_OWNED_ROUTE_REQUIRED",
                        "DECISION_SATURATION_CLOSURE_REQUIRED",
                    ],
                    "sends_message": False, "canonical_route_view": [company],
                }
                result = self.runtime.evaluate_outreach_readiness(self.args)
                self.assertEqual(result["outreach_readiness"], "COMPANY_ROUTE_READY")
                self.assertEqual(result["readiness"], "COMPANY_ROUTE_READY")
                self.assertIn("DECISION_SATURATION_CLOSURE_REQUIRED", result["block_reasons"])
                self.assertNotIn("VERIFIED_ACCOUNT_OWNED_ROUTE_REQUIRED", result["block_reasons"])
                self.assertFalse(result["sends_message"])

    def test_final_named_route_preserves_lower_level_and_non_route_blockers(self):
        named = {
            "route_source": "COMPILED_OBSERVATION", "observation_id": "OBS-NAMED-CAP",
            "kind": "EMAIL", "value": "named@example.invalid", "named_person": "Private Name",
            "verified": True, "current": True, "owned_by_account": True,
            "owner_entity_id": "SYNTH-ACCOUNT-ROUTE-001", "route_scope": "BUYER_DIRECT",
            "evidence_ids": ["EVD-NAMED-CAP"],
        }
        for lower_readiness in ("NAMED_ROUTE_READY", "FOLLOW_UP_READY", "SEND_READY"):
            with self.subTest(lower_readiness=lower_readiness):
                self.runtime.outreach_result = {
                    ...
                    "sends_message": False, "canonical_route_view": [named],
                }
                result = self.runtime.evaluate_outreach_readiness(self.args)
                self.assertEqual(result["outreach_readiness"], lower_readiness)
                self.assertEqual(result["readiness"], lower_readiness)
                self.assertEqual(result["valid_named_route_observation_ids"], ["OBS-NAMED-CAP"])
                self.assertIn("DECISION_SATURATION_CLOSURE_REQUIRED", result["block_reasons"])
                self.assertNotIn("VERIFIED_ACCOUNT_OWNED_ROUTE_REQUIRED", result["block_reasons"])
                self.assertFalse(result["sends_message"])
